...
Pass a Jarvis JWT that your server mints. The SDK skips the `/api/auth/exchange` call and uses the token as-is.
...
| Variable | Description |
|----------|-------------|
| `CUSTOM_JWT_PUBLIC_KEY` | PEM-encoded RSA private key. Jarvis uses this to verify tokens signed by your server. |
